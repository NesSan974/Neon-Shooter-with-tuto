...
namespace neonShooter
{
    class Grid
    {
        ...
        public Vector2 ToVec2(Vector3 v)
        {
            ...
            return (new Vector2(v.X, v.Y) - Game1.ScreenSize / 2f) * factor + Game1.ScreenSize / 2;
        }
            ...
            for (int y = 1; y < height; y++)
            {
                for (int x = 1; x < width; x++)
                {
                    ...
                    if (x > 1 && y > 1)
                    {
                        ...
                        Vector2 mid = Vector2.CatmullRom(ToVec2(points[x - 2, y].Position), left, p, ToVec2(points[clampedX, y].Position), 0.9f);
